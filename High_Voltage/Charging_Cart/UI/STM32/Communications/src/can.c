...
void cell_data_parsing(uint32_t id, SegmentData_t* bms_data, uint8_t* rx_data) {
    uint8_t bms_module = (id - CAN_ID_BMS_DATA_START) / BMS_DATA_MSGS_PER_SEGMENT;
    uint8_t cell_start_index =
        ((id - CAN_ID_BMS_DATA_START) % BMS_DATA_MSGS_PER_SEGMENT) * BMS_DATA_CELLS_PER_MSG;
    for (uint8_t i = 0; i < BMS_DATA_CELLS_PER_MSG; i++) {
        bms_data[bms_module].cell_v[cell_start_index + i] = rx_data[i];
        bms_data[bms_module].temp_C[cell_start_index + i] = rx_data[i + BMS_DATA_CELLS_PER_MSG];
    }
}

void cell_soc_parsing(uint32_t id, SegmentData_t* bms_data, uint8_t* rx_data) {
    uint8_t bms_module = (id - CAN_ID_SOC_DATA_START) / SOC_DATA_MSGS_PER_SEGMENT;
    uint8_t cell_start_index =
        ((id - CAN_ID_SOC_DATA_START) % SOC_DATA_MSGS_PER_SEGMENT) * SOC_DATA_CELLS_PER_MSG;
    for (uint8_t i = 0; i < SOC_DATA_CELLS_PER_MSG; i++) {
        bms_data[bms_module].soc[cell_start_index + i] = rx_data[i];
    }
}

void elcon_to_charger_parsing(uint8_t* rx_data, uint8_t dlc) {
    if (dlc < ELCON_RX_MIN_DLC) return;  // Not enough data for voltage, current, and status
...
void can_init(CAN_HandleTypeDef* hdcan) {
    HAL_GPIO_WritePin(CAN_SLEEP_GPIO_Port, CAN_SLEEP_Pin, GPIO_PIN_RESET);  // enable CAN
                                                                            // transceiver
    // Start FDCAN1
...
void HAL_CAN_RxFifo0MsgPendingCallback(CAN_HandleTypeDef* hcan) {
    ...
    if (id == CAN_ID_BATTERY_PACK) {  // Node ID
        ...
        CAN_Info.nomVoltVal =
            (uint8_t)((CAN_Info.packVoltage / NOM_VOLT_DIVISOR) - NOM_VOLT_OFFSET);
...
bool battery_can(CAN_HandleTypeDef* hcan, uint16_t current, bool balancing) {
    ...
    if (CAN_SendData(hcan, CAN_CHARGING_TO_PACK, data, BATTERY_MSG_DLC, false) != HAL_OK) {
...
bool error_can(CAN_HandleTypeDef* hcan) {
    ...
    return CAN_SendData(hcan, CAN_ID_CHARGER_ERRORS, data, ERROR_MSG_DLC, false) == HAL_OK;
}
...
bool charger_can(CAN_HandleTypeDef* hcan, uint16_t v_lim, uint16_t i_lim, bool charge) {
    ...
    return CAN_SendData(hcan, CAN_ID_CHARGING_TO_ELCON, data, ELCON_MSG_DLC, true) == HAL_OK;
}
...
uint16_t cell_voltage_conversion(uint8_t raw_value) {
    return ((uint16_t)raw_value) * CELL_V_STEP_MV + CELL_V_OFFSET_MV;
}

// output in 0.01C
uint16_t cell_temperature_conversion(int8_t raw_value) {
    return ((uint16_t)raw_value) * CELL_TEMP_STEP;
}

// output in percentage
uint16_t cell_soc_conversion(uint8_t raw_value) {
    return ((uint16_t)raw_value * 100) / CELL_SOC_RAW_MAX;
}

// output in 0.1V
uint16_t pack_voltage_conversion(uint16_t raw_value) { return raw_value / PACK_V_DIVISOR; }

// output in percentage
uint16_t pack_soc_conversion(uint16_t raw_value) {
    return (uint16_t)(((uint32_t)raw_value * 100) / PACK_SOC_RAW_MAX);
}
