...
// returns true if the state is one where the user is currently choosing which value to edit (as
// opposed to actively editing a value)
...
// returns true if the state is one where the user is currently editing a value (as opposed to
// choosing which value to edit)
bool isEditState(UserInputState state) {
    return state == Edit_Current_Limit || state == Edit_Voltage_Limit || state == Edit_Status;
}
...
// returns the selected status path based on the current selected index and chargersm state,
// defaults to STAY if no options or invalid index
uint8_t selectedStatusPathFromIndex(uint16_t selectedIndex, chargersm_obj* chargersm) {
    ...
    uint8_t clampedIndex =
        mod_pos(static_cast<int8_t>(selectedIndex - 1), numOptions) +
        1;  // ensure index is within bounds, +1 to account for options[0] being numOptions
...
int16_t clamp_user_var(int16_t value, InfoType type) {
    ...
    switch (type) {
        ...
        case STATUS:
            ...
            // separately, so we can just return the value without clamping
            return value;
        default:
...
// could also easily implement as returning the new userInputState if that's better (for checking,
// etc) returns true if the status message has changed, false otherwise
bool processUI(UserInputState& userInputState, int16_t* userVars, chargersm_obj* chargersm,
               RotaryEncoder& encoder, Display& display, const char*& uiStatusMsg) {
    ...
    // if we're choosing which value to edit, valid input is rotation to change selection or press
    // to confirm selection and enter edit mode
    if (isChooseState(userInputState)) {
        // on press, move from choose state to edit state for the selected variable
        if (pressed) {
            bool changedStatus = false;
            // special case if we're choosing the status, since that doesn't have a single value but
            // rather multiple options based on the chargersm state
        ...
        // if we're just choosing which value to edit, any rotation input just changes the selection
        // without modifying the value, so we can return early after updating the box highlight
        if (net_enc_delta != 0) {
    ...
    // if we're editing a value, rotation input changes the value and press confirms the change and
    // goes back to choose state
    if (isEditState(userInputState)) {
        ...
        if (pressed) {
            ...
            // special case for status since we need to convert from the selected index to the
            // actual status path based on the chargersm state
            if (userInputState == Edit_Status) {
                userVars[selectedIdx] = selectedStatusPathFromIndex(currentEditValue, chargersm);
            } else {
                userVars[selectedIdx] =
                    clamp_user_var(currentEditValue, static_cast<InfoType>(selectedIdx));
            }
            ...
            uiStatusMsg =
                nullptr;  // clear status message on edit, since it may no longer be relevant after
                          // the change and we don't want to show an outdated one. Will be updated
                          // in the main loop after processing the change.
            return true;
        }
        //
        if (net_enc_delta != 0) {
            ...
            if (userInputState == Edit_Status) {
                ...
                if (options && options[0] > 0) {
                    uint8_t numOptions = options[0];
                    currentEditValue =
                        mod_pos(static_cast<int8_t>(currentEditValue - 1) + net_enc_delta,
                                numOptions) +
                        1;
                    uiStatusMsg = statusChangeMessages[options[currentEditValue]];
                } else {
                ...
                currentEditValue = clamp_user_var(
                    currentEditValue, static_cast<InfoType>(infoFromState(userInputState)));
...
void setFaultMsg(Display& disp, chargersm_obj* me) {
    ...
    // if number of faults has changed, or the current fault index is no longer valid, reset to
    // first fault
