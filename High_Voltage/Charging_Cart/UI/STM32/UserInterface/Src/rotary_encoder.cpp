...
RotaryEncoder::RotaryEncoder(GPIO_TypeDef* A_PORT, uint16_t A_PIN, GPIO_TypeDef* B_PORT,
                             uint16_t B_PIN, GPIO_TypeDef* S_PORT, uint16_t S_PIN)
    : A_PORT(A_PORT),
      A_PIN(A_PIN),
      B_PORT(B_PORT),
      B_PIN(B_PIN),
...
void RotaryEncoder::rotationCallback() {
    uint8_t new_state = (HAL_GPIO_ReadPin(A_PORT, A_PIN) << 1) | HAL_GPIO_ReadPin(B_PORT, B_PIN);
    ...
    */
    ...
    // check the counter agains the threshold and add to buffer if threshold is crossed, then reset
    // counter.
...
bool RotaryEncoder::processBuffer(int8_t& out_delta) {
    ...
    while (temp_buffer != 0) {
        if ((temp_buffer & 0b11) == 0b01) {  // CW
            ...
            out_delta--;
        } else if ((temp_buffer & 0b11) ==
                   0b11) {  // button press, confirm choose and move to edit state
            return true;
        }
