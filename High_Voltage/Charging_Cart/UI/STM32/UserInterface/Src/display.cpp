...
Display::Display(SPI_HandleTypeDef* SPI_HANDLE, GPIO_TypeDef* CS_PORT, const uint16_t CS_PIN,
                 ...
                 const uint16_t RST_PIN, TIM_HandleTypeDef* BKLT_TIMER,
                 const uint32_t BKLT_PWM_CHANNEL)
    : SPI_HANDLE(SPI_HANDLE),
      ...
      RST_PORT(RST_PORT),
      RST_PIN(RST_PIN),
      BKLT_TIMER(BKLT_TIMER),
      BKLT_PWM_CHANNEL(BKLT_PWM_CHANNEL),
...
void Display::init() {
    ...
    uint8_t cmd[] = {
        ...
        CMD_REG_DIVIDER, CMD_VOLUME_MODE, CMD_SET_VOLUME, CMD_POWER_CNFG,
    };
...
void Display::clear() {
    // Initialize framebuffer to base font (static const uint8_t framebufferBase[8][128] in
    // font.hpp)
...
void Display::setInfo(InfoType type, int16_t value) {
    ...
    if (type == CHARGE_PERCENT) {
        ...
        for (uint8_t i = 0; i < PROGRESS_BAR_WIDTH; i++) {
            if (i < barWidth) {
                addToFramebuffer(PROGRESS_BAR_X + i, PROGRESS_BAR_Y, (uint8_t[]){0xFF}, 1,
                                 PROGRESS_BAR_HEIGHT, inverted);
            } else {
                addToFramebuffer(PROGRESS_BAR_X + i, PROGRESS_BAR_Y, (uint8_t[]){0x00}, 1,
                                 PROGRESS_BAR_HEIGHT, inverted);
            }
        }
    }
    ...
    /*
    if (value < 0){
        addToFramebuffer(InfoTypeData[type][3], InfoTypeData[type][1], letters5x7['a'-'a'], 5, 7,
    inverted); addToFramebuffer(InfoTypeData[type][2], InfoTypeData[type][1], letters5x7['t'-'a'],
    5, 7, inverted); } else
     */
    if (fontSize == SMALL_N || fontSize == SMALL_L) {
        while (digit < 4 && InfoTypeData[type][2 + digit] != 0) {
            uint8_t digitValue = (value == 0 && fontSize == SMALL_N && digit != 0)
                                     ? 10
                                     : value % 10;  // show blank for leading 0s if SMALL_N
            addToFramebuffer(InfoTypeData[type][2 + digit], InfoTypeData[type][1],
                             nums3x5[digitValue], 3, 5, inverted);
            value /= 10;
            digit++;
        }
    } else if (fontSize == LARGE_N || fontSize == LARGE_L) {
        while (digit < 4 && InfoTypeData[type][2 + digit] != 0) {
            uint8_t digitValue = (value == 0 && fontSize == LARGE_N && digit != 0)
                                     ? 10
                                     : value % 10;  // show blank for leading 0s if LARGE_N
            addToFramebuffer(InfoTypeData[type][2 + digit], InfoTypeData[type][1],
                             nums5x7[digitValue], 5, 7, inverted);
            value /= 10;
            digit++;
        }
        ...
        while (digit < 4 && InfoTypeData[type][2 + digit] != 0) {
            ...
            addToFramebuffer(InfoTypeData[type][2 + digit], InfoTypeData[type][1],
                             nums3x5[digitValue], 3, 5, inverted, true);
            value /= 10;
            digit++;
        }
    }
...
void Display::setStatus(const char* msg, uint8_t offset, bool fill_spaces) {
    ...
    while (cursorx <= max_cursorx) {
        ...
        if (cur >= 'A' && cur <= '^') {
            addToFramebuffer(cursorx, cursory, letters5x7[static_cast<uint8_t>(cur - 'A')],
                             charSize, height, isInverted(STATUS));
        } else if (cur >= '0' && cur <= '9') {
            // number
            addToFramebuffer(cursorx, cursory, nums5x7[static_cast<uint8_t>(cur - '0')], charSize,
                             height, isInverted(STATUS));
        } else if (cur == ' ') {
...
void Display::setStatus(const char* msg) { setStatus(msg, 0, true); }
...
// for testing, sets all info types to the same value to easily check for bounding box and highlight
// issues
...
// private functions
void Display::addToFramebuffer(uint8_t x, uint8_t y, const uint8_t* data, uint8_t width,
                               uint8_t height, bool inverted, bool doubled) {
    if (doubled) {
        ...
        // note: these upper and lower refer to the upper and lower parts of the bytes that store
        // the double-sized character, not the display pages the LSB is higher than the MSB
        uint8_t doubleDataUpper[width * 2];
        uint8_t doubleDataLower[width * 2];
        for (uint8_t i = 0; i < width; i++) {
            doubleDataUpper[2 * i] = doubleDataUpper[2 * i + 1] =
                doubledBits[(data[i] >> 4) & 0x0F];
            doubleDataLower[2 * i] = doubleDataLower[2 * i + 1] =
                doubledBits[(data[i] & 0x0F)];  // Lower 4 bits for lower page
        }

        // if height*2 is greater than one page, we need to split it into two pages
        if (height * 2 > 8) {
            addToFramebuffer(x, y, doubleDataUpper, width * 2, height * 2 - 8,
                             inverted);  // Upper page
            addToFramebuffer(x, y - (height * 2 - 8), doubleDataLower, width * 2, 8,
                             inverted);  // Lower page
        } else {
        ...
        if (numOnUpperPage <= 0) {
            for (uint8_t col = x; col < x + width; col++) {
                ...
                framebuffer[lower_page][col] &=
                    (0xFF << (numOnLowerPage) | 0xFF >> (8 + numOnUpperPage));
                // set the bits in range on lower page
                framebuffer[lower_page][col] |= ((data[col - x] ^ invertMask) & dataGuard)
                                                << (numOnLowerPage - height);
            }
...
void Display::getBoundingBox(InfoType type, uint8_t& left, uint8_t& bottom, uint8_t& right,
                             uint8_t& top) {
    // MSG is different
...
bool Display::isInverted(InfoType type) { return infoInvertedMask & (1 << type); }
