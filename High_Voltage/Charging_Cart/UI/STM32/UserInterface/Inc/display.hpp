...
class Display {
   public:
    Display(SPI_HandleTypeDef* SPI_HANDLE, GPIO_TypeDef* CS_PORT, const uint16_t CS_PIN,
            GPIO_TypeDef* A0_PORT, const uint16_t A0_PIN, GPIO_TypeDef* RST_PORT,
            const uint16_t RST_PIN, TIM_HandleTypeDef* BKLT_TIMER, const uint32_t BKLT_PWM_CHANNEL);
    ~Display();
   ...
   private:
    ...
    void addToFramebuffer(uint8_t x, uint8_t y, const uint8_t* data, uint8_t width, uint8_t height,
                          bool inverted = false, bool doubled = false);
