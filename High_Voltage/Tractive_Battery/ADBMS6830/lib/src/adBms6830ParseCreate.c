 ...
 *******************************************************************************
 */
void SetConfigB_DischargeTimeOutValue(uint8_t tIC, cell_asic* ic, DTRNG timer_rang,
                                      DCTO timeout_value) {
...
void adBms6830ParseConfiga(uint8_t tIC, cell_asic* ic, uint8_t* data) {
    ...
    for (uint8_t curr_ic = 0; curr_ic < tIC; curr_ic++) {
        ...
        ic[curr_ic].rx_cfga.gpo = ((ic[curr_ic].configa.rx_data[3] & 0xFF) |
                                   ((ic[curr_ic].configa.rx_data[4] & 0x03) << 8));
...
void adBms6830ParseConfigb(uint8_t tIC, cell_asic* ic, uint8_t* data) {
    ...
    for (uint8_t curr_ic = 0; curr_ic < tIC; curr_ic++) {
        ...
        ic[curr_ic].rx_cfgb.vuv =
            ((ic[curr_ic].configb.rx_data[0]) | ((ic[curr_ic].configb.rx_data[1] & 0x0F) << 8));
        ic[curr_ic].rx_cfgb.vov =
            (ic[curr_ic].configb.rx_data[2] << 4) + ((ic[curr_ic].configb.rx_data[1] & 0xF0) >> 4);
        ...
        ic[curr_ic].rx_cfgb.dcc =
            ((ic[curr_ic].configb.rx_data[4]) | ((ic[curr_ic].configb.rx_data[5] & 0xFF) << 8));
    }
}
...
void adBms6830ParseAux(uint8_t tIC, cell_asic* ic, GRP grp, uint8_t* aux_data) {
    uint8_t *data, data_size, address = 0;
    if (grp == ALL_GRP) {
        data_size = (RDASALL_SIZE - 44);
    } /* RDASALL_SIZE 68 byte - (RAUX 20 byte + STATUS 24 byte) */
    else {
        data_size = RX_DATA;
    }
    data = (uint8_t*)calloc(data_size, sizeof(uint8_t));
...
void adBms6830ParseRAux(uint8_t tIC, cell_asic* ic, GRP grp, uint8_t* raux_data) {
    uint8_t *data, data_size, address = 0;
    if (grp == ALL_GRP) {
        data_size = (RDASALL_SIZE - 48);
    } /* RDASALL_SIZE 68 byte - (AUX 24 byte + STATUS 24 byte) */
    else {
        data_size = RX_DATA;
    }
    data = (uint8_t*)calloc(data_size, sizeof(uint8_t));
...
void adBms6830ParseStatusA(uint8_t tIC, cell_asic* ic, uint8_t* data) {
    ...
    for (uint8_t curr_ic = 0; curr_ic < tIC; curr_ic++) {
        ...
        address = ((curr_ic + 1) * (RX_DATA));
        ic[curr_ic].stata.vref2 =
            (ic[curr_ic].stat.rx_data[0] | (ic[curr_ic].stat.rx_data[1] << 8));
        ic[curr_ic].stata.itmp = (ic[curr_ic].stat.rx_data[2] | (ic[curr_ic].stat.rx_data[3] << 8));
        ic[curr_ic].stata.vref3 =
            (ic[curr_ic].stat.rx_data[4] | (ic[curr_ic].stat.rx_data[5] << 8));
    }
}
...
void adBms6830ParseStatusE(uint8_t tIC, cell_asic* ic, uint8_t* data) {
    ...
    for (uint8_t curr_ic = 0; curr_ic < tIC; curr_ic++) {
        memcpy(&ic[curr_ic].stat.rx_data[0], &data[address], RX_DATA); /* dst , src , size */
        address = ((curr_ic + 1) * (RX_DATA));
        ic[curr_ic].state.gpi =
            ((ic[curr_ic].stat.rx_data[4] + ((ic[curr_ic].stat.rx_data[5] & 0x03) << 8)));
...
void adBms6830CreateConfiga(uint8_t tIC, cell_asic* ic) {
    for (uint8_t curr_ic = 0; curr_ic < tIC; curr_ic++) {
        ic[curr_ic].configa.tx_data[0] =
            (((ic[curr_ic].tx_cfga.refon & 0x01) << 7) | (ic[curr_ic].tx_cfga.cth & 0x07));
        ic[curr_ic].configa.tx_data[1] = (ic[curr_ic].tx_cfga.flag_d & 0xFF);
        ic[curr_ic].configa.tx_data[2] =
            (((ic[curr_ic].tx_cfga.soakon & 0x01) << 7) |
             ((ic[curr_ic].tx_cfga.owrng & 0x01) << 6) | ((ic[curr_ic].tx_cfga.owa & 0x07) << 3));
        ic[curr_ic].configa.tx_data[3] = ((ic[curr_ic].tx_cfga.gpo & 0x00FF));
        ic[curr_ic].configa.tx_data[4] = ((ic[curr_ic].tx_cfga.gpo & 0x0300) >> 8);
        ic[curr_ic].configa.tx_data[5] =
            (((ic[curr_ic].tx_cfga.snap & 0x01) << 5) |
             ((ic[curr_ic].tx_cfga.mute_st & 0x01) << 4) |
             ((ic[curr_ic].tx_cfga.comm_bk & 0x01) << 3) | (ic[curr_ic].tx_cfga.fc & 0x07));
    }
}
...
void adBms6830CreateConfigb(uint8_t tIC, cell_asic* ic) {
    for (uint8_t curr_ic = 0; curr_ic < tIC; curr_ic++) {
        ...
        ic[curr_ic].configb.tx_data[1] =
            (((ic[curr_ic].tx_cfgb.vov & 0x000F) << 4) | ((ic[curr_ic].tx_cfgb.vuv) >> 8));
        ic[curr_ic].configb.tx_data[2] = ((ic[curr_ic].tx_cfgb.vov >> 4) & 0x0FF);
        ic[curr_ic].configb.tx_data[3] =
            (((ic[curr_ic].tx_cfgb.dtmen & 0x01) << 7) | ((ic[curr_ic].tx_cfgb.dtrng & 0x01) << 6) |
             ((ic[curr_ic].tx_cfgb.dcto & 0x3F) << 0));
...
void adBms6830CreateClrflagData(uint8_t tIC, cell_asic* ic) {
    for (uint8_t curr_ic = 0; curr_ic < tIC; curr_ic++) {
        ...
        ic[curr_ic].clrflag.tx_data[4] =
            ((ic[curr_ic].clflag.cl_vaov << 7) | (ic[curr_ic].clflag.cl_vauv << 6) |
             (ic[curr_ic].clflag.cl_vdov << 5) | (ic[curr_ic].clflag.cl_vduv << 4) |
             (ic[curr_ic].clflag.cl_ced << 3) | (ic[curr_ic].clflag.cl_cmed << 2) |
             (ic[curr_ic].clflag.cl_sed << 1) | (ic[curr_ic].clflag.cl_smed));
        ic[curr_ic].clrflag.tx_data[5] =
            ((ic[curr_ic].clflag.cl_vde << 7) | (ic[curr_ic].clflag.cl_vdel << 6) |
             (ic[curr_ic].clflag.cl_spiflt << 4) | (ic[curr_ic].clflag.cl_sleep << 3) |
             (ic[curr_ic].clflag.cl_thsd << 2) | (ic[curr_ic].clflag.cl_tmode << 1) |
             (ic[curr_ic].clflag.cl_oscchk));
    }
}
...
void adBms6830CreateComm(uint8_t tIC, cell_asic* ic) {
    for (uint8_t curr_ic = 0; curr_ic < tIC; curr_ic++) {
        ic[curr_ic].com.tx_data[0] =
            ((ic[curr_ic].comm.icomm[0] & 0x0F) << 4 | (ic[curr_ic].comm.fcomm[0] & 0x0F));
        ic[curr_ic].com.tx_data[1] = ((ic[curr_ic].comm.data[0]));
        ic[curr_ic].com.tx_data[2] =
            ((ic[curr_ic].comm.icomm[1] & 0x0F) << 4) | (ic[curr_ic].comm.fcomm[1] & 0x0F);
        ic[curr_ic].com.tx_data[3] = ((ic[curr_ic].comm.data[1]));
        ic[curr_ic].com.tx_data[4] =
            ((ic[curr_ic].comm.icomm[2] & 0x0F) << 4 | (ic[curr_ic].comm.fcomm[2] & 0x0F));
...
void adBms6830CreatePwma(uint8_t tIC, cell_asic* ic) {
    for (uint8_t curr_ic = 0; curr_ic < tIC; curr_ic++) {
        ic[curr_ic].pwma.tx_data[0] =
            ((ic[curr_ic].PwmA.pwma[1] & 0x0F) << 4 | (ic[curr_ic].PwmA.pwma[0] & 0x0F));
        ic[curr_ic].pwma.tx_data[1] =
            ((ic[curr_ic].PwmA.pwma[3] & 0x0F) << 4 | (ic[curr_ic].PwmA.pwma[2] & 0x0F));
        ic[curr_ic].pwma.tx_data[2] =
            ((ic[curr_ic].PwmA.pwma[5] & 0x0F) << 4 | (ic[curr_ic].PwmA.pwma[4] & 0x0F));
        ic[curr_ic].pwma.tx_data[3] =
            ((ic[curr_ic].PwmA.pwma[7] & 0x0F) << 4 | (ic[curr_ic].PwmA.pwma[6] & 0x0F));
        ic[curr_ic].pwma.tx_data[4] =
            ((ic[curr_ic].PwmA.pwma[9] & 0x0F) << 4 | (ic[curr_ic].PwmA.pwma[8] & 0x0F));
        ic[curr_ic].pwma.tx_data[5] =
            ((ic[curr_ic].PwmA.pwma[11] & 0x0F) << 4 | (ic[curr_ic].PwmA.pwma[10] & 0x0F));
    }
}
/**
 *******************************************************************************
...
void adBms6830CreatePwmb(uint8_t tIC, cell_asic* ic) {
    for (uint8_t curr_ic = 0; curr_ic < tIC; curr_ic++) {
        ic[curr_ic].pwmb.tx_data[0] =
            ((ic[curr_ic].PwmB.pwmb[1] & 0x0F) << 4 | (ic[curr_ic].PwmB.pwmb[0] & 0x0F));
        ic[curr_ic].pwmb.tx_data[1] =
            ((ic[curr_ic].PwmB.pwmb[3] & 0x0F) << 4 | (ic[curr_ic].PwmB.pwmb[2] & 0x0F));
    }
}
/** @}*/
/** @}*/
