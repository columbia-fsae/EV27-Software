...
KalmanSOC g_kalman_soc[TOTAL_MODULES][CELLS_PER_MOD];
...
void adbms_main_init(volatile CAN_Inputs_t* can_data) {
    ...
    for (uint8_t cic = 0; cic < TOTAL_IC; cic++) {
        uint64_t raw_id =
            (((uint64_t)IC[cic].sid.sid[5]) << 40U) | (((uint64_t)IC[cic].sid.sid[4]) << 32U) |
            (((uint64_t)IC[cic].sid.sid[3]) << 24U) | (((uint64_t)IC[cic].sid.sid[2]) << 16U) |
            (((uint64_t)IC[cic].sid.sid[1]) << 8U) | (uint64_t)IC[cic].sid.sid[0];
...
void adBms6830_soc_init(void) {
    ...
    for (int cic = 0; cic < TOTAL_MODULES; cic++) {
        ...
        for (int i = 0; i < CELLS_PER_MOD; i++) {
            ...
            bool from_flash =
                SOC_Flash_Load(&persistent);  // TODO: per-cell flash address when scaling
            ...
            */
            ...
            //        i + 1, (float) PackSegments[cic].cell_v_mV[i]/1000.0f, (float)
            //        PackSegments[cic].temp_C[temp_idx], g_kalman_soc[cic][i].x.soc * 100.0f);
            g_soc_initialized[cic][i] = true;
            //}

            // Initialize RC lookup tables from above
            KalmanSOC_SetRCLookupTables(&g_kalman_soc[cic][i], &g_r_ct_lut, &g_c_ct_lut,
...
void adBms6830_soc_update(int cic, int cell, float cell_voltage, float pack_current,
    ...
    if (success) {
        ...
        if (g_soc_update_counter[cic][cell] >= 100) {
            ...
            bool flash_ok = SOC_Flash_Save(&persistent); // TODO: unique address per cic/cell when
        scaling if (flash_ok)
...
void adBms6830_print_soc_estimate(int cic, int cell, const SOC_Estimate* est) {
    ...
    printf(" Cell %02d: SOC=%5.1f%%  OCV=%.3fV  R0=%.4f  Cap=%.2fAh  SOH=%5.1f%%\r\n", cell + 1,
           est->soc_percent, est->voltage_ocv, est->r0, est->capacity_ah, est->soh_percent);
}
