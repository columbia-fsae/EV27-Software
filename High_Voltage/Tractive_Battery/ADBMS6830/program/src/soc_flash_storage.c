...
bool SOC_Flash_Save(const KalmanSOC_PersistentState* persistent) {
    ...
    for (uint32_t i = 0; i < num_dwords; i++) {
        status = HAL_FLASH_Program(FLASH_TYPEPROGRAM_DOUBLEWORD, SOC_FLASH_ADDRESS + (i * 8),
                                   data_ptr[i]);
        if (status != HAL_OK) {
            HAL_FLASH_Lock();
            return false;
...
bool SOC_Flash_Load(KalmanSOC_PersistentState* persistent) {
    // Read from flash
    const KalmanSOC_PersistentState* flash_data =
        (const KalmanSOC_PersistentState*)SOC_FLASH_ADDRESS;
