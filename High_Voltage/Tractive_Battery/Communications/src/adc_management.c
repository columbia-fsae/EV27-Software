...
// Temperature ADC Sensor Lookup Data Structure (T, R(T)/R(25))
const float TSENSE_LOOKUP[ADC_TSENSE_LOOKUP_SIZE][2] = ADC_TSENSE_LOOKUP_DATA;

// ADC Buffers
static volatile uint16_t rawADC1Values[ADC1_CHANNEL_COUNT];
static volatile uint16_t rawADC2Values[ADC2_CHANNEL_COUNT];
...
void adc_init(ADC_HandleTypeDef* hadc1, ADC_HandleTypeDef* hadc2) {
    ...
    HAL_ADC_Start_DMA(hadc1, (uint32_t*)rawADC1Values, ADC1_CHANNEL_COUNT);
    ...
    HAL_DMA_Start(hadc2->DMA_Handle, (uint32_t)&hadc2->Instance->DR, (uint32_t)rawADC2Values,
                  ADC2_CHANNEL_COUNT);

    hadc2->Instance->CR |= ADC_CR_ADEN;
    uint32_t tickstart = HAL_GetTick();
    while (!(hadc2->Instance->ISR & ADC_ISR_ADRDY)) {
        if (HAL_GetTick() - tickstart > ADC2_ENABLE_TIMEOUT_MS) {
            Error_Handler();
        }
    }
...
float temp_convert(uint16_t adc_value) {
    ...
    // Clamping Output
    return (rntc - TSENSE_LOOKUP[i - 1][1]) / (TSENSE_LOOKUP[i][1] - TSENSE_LOOKUP[i - 1][1]) *
               (TSENSE_LOOKUP[i][0] - TSENSE_LOOKUP[i - 1][0]) +
           TSENSE_LOOKUP[i - 1][0];
}
