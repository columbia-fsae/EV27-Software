...
/* Private define ------------------------------------------------------------*/
/* USER CODE BEGIN PD */
// Reset Counters to rest the microcontroller in case of critical faults, will get stuck after a set
// number of resets
...
#define RUN_EVERY(tick, interval, offset, curr_tick)                  \
    for (uint8_t _run = (curr_tick + (offset) >= (interval) + (tick)) \
                            ? ((tick) = curr_tick + (offset), 1)      \
                            : 0;                                      \
         _run; _run = 0)
...
static void MX_GPIO_Init(void) {
    ...
    HAL_GPIO_WritePin(GPIOB, IR_Plus_EN_Pin | Precharge_EN_Pin | IR_Minus_EN_Pin | GPIO_PIN_6,
                      GPIO_PIN_RESET);
    ...
    /*Configure GPIO pins : IR_Plus_EN_Pin Precharge_EN_Pin IR_Minus_EN_Pin PB5
                             PB6 */
    GPIO_InitStruct.Pin =
        IR_Plus_EN_Pin | Precharge_EN_Pin | IR_Minus_EN_Pin | GPIO_PIN_5 | GPIO_PIN_6;
