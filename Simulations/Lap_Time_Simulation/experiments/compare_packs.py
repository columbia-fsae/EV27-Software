...
def evaluate_pack(pack_combo: tuple[int], mass: int) -> dict:
    ...
    try:
        ...
        car.battery._T = (
            TEMP_START  # realistic pre-warmed/hot-day start, not the sim's cold-start 25 C default
        )
        ...
        temp_ref_fn = linear_temp_schedule(
            total_distance, temp_start=TEMP_START, temp_end=TEMP_END, derate=TEMP_DERATE
        )
        endur_car = car.replace(
            ecms=EcmsController(
                soc_ref_fn=soc_ref_fn,
                temp_ref_fn=temp_ref_fn,
                k=ECMS_K,
                k_temp=ECMS_K_TEMP,
                s1_kp=ECMS_S1_KP,
                s1_ki=ECMS_S1_KI,
        ...
        return {
            ...
            "parallel": parallel,
            "mass_kg": car.mass,
            "batt_mass": car.battery._batt_mass,
            "accel_time": accel_time,
            "skidpad_time": skidpad_time,
            "endur_time": endur_time,
            "energy_wh": energy_wh,
            "final_soc": final_soc,
            "pack_energy_kwh": car.battery.pack_energy,
            "final_energy_kwh": endur.stats.batt_energy,
        ...
        return {
            ...
            "energy_wh": np.nan,
            "final_soc": np.nan,
            "pack_energy_kwh": np.nan,
            "final_energy_kwh": np.nan,
            "total_points": np.nan,
...
def evaluate_grid_cell(pack_combo, mass):
    ...
    print(
        f"[compare_packs]   -> mass={result['mass_kg']:.1f} kg, points={pts:.1f}, \
        score_breakdown = {result['score_breakdown']}, accel_time={result['accel_time']}, \
        skidpad_time = {result['skidpad_time']}, endur_time = {result['endur_time']}"
        if not np.isnan(pts)
        else "[compare_packs]   -> failed, see above"
    )
    return result
...
def main():
    ...
    print(
        f"[compare_packs] sweeping {len(pack_combos)} pack configs x {len(MASS_VALUES)} masses = "
        f"{n_combos} configurations (each a full 22-lap endurance run) across {N_WORKERS} \
            worker process(es)..."
    )

    df = grid_sweep(
        {"pack_combo": pack_combos, "mass": MASS_VALUES},
        evaluate_grid_cell,
        n_workers=N_WORKERS,
    )
    print(
    ...
    sp_pivot = df.pivot_table(
        index="parallel", columns="series", values="total_points", aggfunc="max"
    )
    plotting.plot_heatmap(
        sp_pivot.columns.values,
        sp_pivot.index.values,
        sp_pivot.values,
        "Series cells",
        "Parallel cells",
        "Total points (best over mass sweep)",
    ...
    contour = ax2.tricontourf(
        energy_wh, df["mass_kg"], df["total_points"], levels=20, cmap="viridis"
    )
    ax2.scatter(energy_wh, df["mass_kg"], c="k", s=6, alpha=0.3)
    ax2.set_xlabel(
        f"{'Endurance energy consumed' if ENERGY_METRIC == 'final' else 'Nominal pack energy'} (Wh)"
    )
    ...
    scatter3d = ax3.scatter(
        energy_wh, df["mass_kg"], cell_count, c=df["total_points"], cmap="viridis"
    )
    ax3.set_xlabel(
        f"{'Endurance energy consumed' if ENERGY_METRIC == 'final' else 'Nominal pack energy'} (Wh)"
    )
    ...
    combo_energy = (
        df.groupby(["series", "parallel"])["pack_energy_kwh"]
        .first()
        .reset_index()
        .sort_values("pack_energy_kwh")
    ...
    for _, row in selected_combos.iterrows():
        ...
        try:
            ...
            ax4.errorbar(
                mean_energy, mean_points, xerr=e_err, yerr=p_err, fmt="o", capsize=4, label=label
            )
            ax4.annotate(
                f"{design_e:.0f} Wh",
                (mean_energy, mean_points),
            ...
            print(
                f"[compare_packs] fig4: mass error bars failed for {label} ({exc}); "
            ...
            ax4.annotate(
                f"{design_e:.0f} Wh",
                (single[energy_col] * 1000.0, single["total_points"]),
                textcoords="offset points",
                xytext=(6, 6),
                fontsize=7,
            )
    ...
    ax4.set_title(
        ...
        f"+{FIG4_MASS_OFFSETS_KG[1]} to +{FIG4_MASS_OFFSETS_KG[-1]} kg; \
            point labels = nameplate energy)"
    )
    ...
    if fig4_records:
        ...
        print(
            "[compare_packs] fig5: no combo had a full mass sweep to show "
            "mass/points variance for; skipping"
        )

    best = df.loc[df["total_points"].idxmax()]
    print(
        f"[compare_packs] best: series={int(best.series)} parallel={int(best.parallel)} "
