...
class EcmsController:
    ...
    k: float = 100.0  # tracking-cost weight on (Ir-I)^2; same units as r0 (Ohms)
    ...
    def command_current(
        self, battery: Battery, i_request: float, v_ocv: float, r0: float, dt: float
    ) -> float:
        ...
        if not np.isfinite(dt):
            return 0.0
        i_star = (1 / (self.k + 2 * self.k_temp * r0 * self.s2)) * (
            self.k * i_request - self.s1 * v_ocv
        )
        i_max = (
            battery.available_power() / battery.voltage
        )  # pack-current ceiling, reuses the existing headroom check
    ...
    def update_s1(self, soc_actual: float, dx: float, dt: float) -> None:
        ...
        if not np.isfinite(dt):
            return
        e = (
            self.soc_ref_fn(self.s1_distance_traveled) - soc_actual
        )  # > 0 => behind schedule, used too much already
        self.s1_integral += e * dt
        self.s1 = self.s1_0 + float(
            np.clip(self.s1_kp * e + self.s1_ki * self.s1_integral, 0.0, self.s1_max)
        )
    ...
    def update_s2(self, temp_actual: float, dx: float, dt: float) -> None:
        ...
        if not np.isfinite(dt):
            return
        e = temp_actual - self.temp_ref_fn(
            self.s2_distance_traveled
        )  # > 0 => hotter than the budget allows at this distance
        self.s2_integral += e * dt
        self.s2 = self.s2_0 + float(
            np.clip(self.s2_kp * e + self.s2_ki * self.s2_integral, 0.0, self.s2_max)
        )
