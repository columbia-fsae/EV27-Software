...
int main(int argc, char* argv[]) {
    ...
    if (cols.temperature_idx < 0) {
        fprintf(stderr,
                "NOTE: no temperature column found -- defaulting to 25.0 C for every row\n");
    }

    fprintf(fout,
            "time_ms,input_voltage,input_current,input_temperature,"
    ...
    while (fgets(raw, sizeof(raw), fin)) {
        ...
        if (!initialized) {
            ...
            KalmanSOC_SetRCLookupTables(&kf, &g_r_ct_lut, &g_c_ct_lut, &g_r_dif_lut, &g_c_dif_lut,
                                        &g_r0_lut);
            initialized = true;
        ...
        if (ok) {
            fprintf(fout, "%.3f,%.6f,%.6f,%.3f,%.6f,%.6f,%.8f,%.6f,%.6f,%.8f,%.8f,%.8f,%d,%d\n",
                    time_ms, voltage, current, temperature, estimate.soc_percent,
                    estimate.voltage_ocv, estimate.r0, estimate.capacity_ah, estimate.soh_percent,
                    estimate.v_ct, estimate.v_dif, estimate.uncertainty, estimate.valid ? 1 : 0,
                    estimate.bypassed ? 1 : 0);
        } else {
            fprintf(stderr,
                    "WARNING: row %ld: KalmanSOC_Update() rejected the update "
